...
test_that("pdRaster works",{
  ...
  dmp = capture_output(expect_is(pdRaster(r3, un3), "SpatRaster"))
...
pdr = suppressWarnings(pdRaster(r, unknown = un, mask = naMap, genplot = FALSE))
wd = wDist(pdr, site)

test_that("wDist works",{
  expect_is(wd, "wDist")
